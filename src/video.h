...
ClipState *get_clip(AppState *state, int64_t pos_us, int64_t *last_start_us) {
	int64_t start_us = 0;
	ClipState *clip = &dyn_get(&state->clips, 0);
	...
	for (int i = 0; i < state->clips.size; i++) {
		clip = &dyn_get(&state->clips, i);

		if (pos_us >= start_us && pos_us <= start_us + clip->duration_us) {
			*last_start_us = start_us;
			return clip;
		}
...
void *decode_video(void *userdata) {
	...
	for (int i = 0; i < state->clips.size; i++) {
		...
		if (avformat_open_input(&clip->ps.fmt_ctx, clip->file_path, NULL, NULL) < 0) {
			printf("unable to open clip %s\n", clip->file_path);
			return NULL;
		}

		if (avformat_find_stream_info(clip->ps.fmt_ctx, NULL) < 0) {
			printf("unable to find stream info?\n");
			return NULL;
		}

		const AVCodec *audio_codec = NULL;
		int audio_stream_idx = av_find_best_stream(clip->ps.fmt_ctx, AVMEDIA_TYPE_AUDIO, -1, -1, &audio_codec, 0);
		if (audio_stream_idx < 0) {
			printf("Cannot find an audio stream\n");
			return NULL;
		}
		AVStream *audio_stream = clip->ps.fmt_ctx->streams[audio_stream_idx];
		...
		if (swr_init(clip->ps.swr) < 0) {
			printf("failed to init audio resampling\n");
			return NULL;
		}

		const AVCodec *video_codec = NULL;
		int video_stream_idx = av_find_best_stream(clip->ps.fmt_ctx, AVMEDIA_TYPE_VIDEO, -1, -1, &video_codec, 0);
		if (video_stream_idx < 0) {
			printf("Cannot find an video stream\n");
			return NULL;
		}

		AVStream *video_stream = clip->ps.fmt_ctx->streams[video_stream_idx];
		...
		clip->duration_us = ((double)clip->ps.fmt_ctx->duration / AV_TIME_BASE) * 1000000.0;
		clip->ps.sws_ctx = sws_getContext(video_ctx->width, video_ctx->height, video_ctx->pix_fmt, pb->width, pb->height, pb->pix_fmt, SWS_LANCZOS, NULL, NULL, NULL);
		clip->ps.video_ctx = video_ctx;
		clip->ps.video_stream_idx = video_stream_idx;
		clip->ps.video_stream = video_stream;

		clip->ps.audio_ctx = audio_ctx;
		clip->ps.audio_stream_idx = audio_stream_idx;
		clip->ps.audio_stream = audio_stream;
	}
	...
	while (!state->quit) {
		...
		ClipState *clip = get_clip(state, pb->cur_time_us, &last_start_us);

		if (pb->should_seek) {
			avcodec_flush_buffers(clip->ps.video_ctx);
			avcodec_flush_buffers(clip->ps.audio_ctx);

			clip = get_clip(state, pb->seek_time_us, &last_start_us);
			int64_t adj_seek_time_us = pb->seek_time_us - last_start_us;

			if (avformat_seek_file(clip->ps.fmt_ctx, -1, INT64_MIN, adj_seek_time_us, INT64_MAX, AVSEEK_FLAG_ANY) < 0) {
		...
		if (av_read_frame(clip->ps.fmt_ctx, pkt) < 0) {
			sleep_ns(1000);
			continue;
		}
		if (pkt->stream_index == clip->ps.video_stream_idx) {
			if (avcodec_send_packet(clip->ps.video_ctx, pkt) < 0) {
				continue;
			}
			...
			do {
				...
				ret = avcodec_receive_frame(clip->ps.video_ctx, video_frame);
				...
				if (video_frame->best_effort_timestamp != AV_NOPTS_VALUE) {
					pts = av_rescale_q(video_frame->best_effort_timestamp, clip->ps.video_stream->time_base, AV_TIME_BASE_Q);
				}
		...
		} else if (pkt->stream_index == clip->ps.audio_stream_idx) {
			if (avcodec_send_packet(clip->ps.audio_ctx, pkt) < 0) {
			...
			int ret = 0;
			do {
				ret = avcodec_receive_frame(clip->ps.audio_ctx, frame);
				if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
					break;
				} else if (ret < 0) {
				...
				if (frame->best_effort_timestamp != AV_NOPTS_VALUE) {
					pts = av_rescale_q(frame->best_effort_timestamp, clip->ps.audio_stream->time_base, AV_TIME_BASE_Q);
				}
				if (pb->sample_rate != clip->ps.audio_ctx->sample_rate) {
					printf("TODO: pts calc needs to get fixed for new sample rate\n");
					printf("pb rate: %d != ctx rate: %d\n", pb->sample_rate, clip->ps.audio_ctx->sample_rate);
					exit(1);
				}

				int dst_samples = av_rescale_rnd(
					swr_get_delay(clip->ps.swr, clip->ps.audio_ctx->sample_rate) + frame->nb_samples,
					pb->sample_rate,
					clip->ps.audio_ctx->sample_rate,
					AV_ROUND_UP
				);
				...
				int dst_chan_sample_count = swr_convert(clip->ps.swr, &audio_buf, dst_samples * pb->channels, (const uint8_t **)frame->data, frame->nb_samples);
...
bool transcode_video(AppState *state) {
	...
	const AVCodec *out_video_codec = avcodec_find_encoder(AV_CODEC_ID_H264);
	if (!out_video_codec) {
	...
	out_video_ctx->height = tr->height;
	out_video_ctx->width  = tr->width;
	out_video_ctx->sample_aspect_ratio = (AVRational){1, 1};
	out_video_ctx->time_base = (AVRational){1, 24}; //av_inv_q(in_video_ctx->framerate);
/*
...
*/
	...
	out_video_ctx->pix_fmt = tr->pix_fmt;
	...
	const AVCodec *out_audio_codec = avcodec_find_encoder(AV_CODEC_ID_AAC);
	if (!out_audio_codec) {
		printf("Failed to get audio encoder!\n");
		return false;
	}
	AVCodecContext *out_audio_ctx = avcodec_alloc_context3(out_audio_codec);
	out_audio_ctx->sample_rate = tr->sample_rate;
	if (av_channel_layout_copy(&out_audio_ctx->ch_layout, &tr->ch_layout) < 0) {
		printf("Failed to copy audio channel layout\n");
		return false;
	}

	out_audio_ctx->sample_fmt = tr->sample_fmt;
	...
	for (int i = 0; i < state->clips.size; i++) {
		...
		if (avformat_open_input(&clip->ts.in_fmt_ctx, clip->file_path, NULL, NULL) < 0) {
			printf("unable to open clip %s\n", clip->file_path);
			return false;
		}

		if (avformat_find_stream_info(clip->ts.in_fmt_ctx, NULL) < 0) {
			printf("unable to find stream info?\n");
			return false;
		}
		...
		clip->ts.in_video_stream_idx = av_find_best_stream(clip->ts.in_fmt_ctx, AVMEDIA_TYPE_VIDEO, -1, -1, &in_video_codec, 0);
		if (clip->ts.in_video_stream_idx < 0) {
			printf("Cannot find an video stream\n");
			return false;
		}
		AVStream *in_video_stream = clip->ts.in_fmt_ctx->streams[clip->ts.in_video_stream_idx];
		...
		in_video_ctx->framerate = av_guess_frame_rate(clip->ts.in_fmt_ctx, in_video_stream, NULL);
		...
		clip->ts.streams[0] = (StreamContext){
			.in_ctx    = in_video_ctx,
		...
		if (!setup_video_filter(&clip->ts.streams[0])) {
			return false;
		}

		// Audio Stack
		const AVCodec *in_audio_codec = NULL;
		clip->ts.in_audio_stream_idx = av_find_best_stream(clip->ts.in_fmt_ctx, AVMEDIA_TYPE_AUDIO, -1, -1, &in_audio_codec, 0);
		if (clip->ts.in_audio_stream_idx < 0) {
			printf("Cannot find an audio stream\n");
			return false;
		}
		AVStream *in_audio_stream = clip->ts.in_fmt_ctx->streams[clip->ts.in_audio_stream_idx];
		...
		clip->ts.streams[1] = (StreamContext){
			.in_ctx    = in_audio_ctx,
		...
		if (!setup_audio_filter(&clip->ts.streams[1])) {
			return false;
		}
	}
	...
	for (int i = 0; i < state->clips.size; i++) {
		...
		while (!state->quit) {
			if (av_read_frame(clip->ts.in_fmt_ctx, pkt) < 0) {
				break;
			}
			...
			if (pkt->stream_index == clip->ts.in_video_stream_idx) {
				s = &clip->ts.streams[out_video_stream_idx];
				prev_pts = &prev_video_pts;
				pts_offset = tr->last_video_pts;
			} else if (pkt->stream_index == clip->ts.in_audio_stream_idx) {
				s = &clip->ts.streams[out_audio_stream_idx];
				prev_pts = &prev_audio_pts;
				pts_offset = tr->last_audio_pts;
			}
			if (!s) {
				av_packet_unref(pkt);
				continue;
			...
			do {
				...
				s->in_frame->pts = pts_offset + s->in_frame->best_effort_timestamp;
				*prev_pts = s->in_frame->pts + s->in_frame->duration;
		...
		for (int i = 0; i < ELEM_COUNT(clip->ts.streams); i++) {
			StreamContext *s = NULL;
			int64_t *prev_pts = NULL;
			int64_t pts_offset = 0;
		...
		tr->last_audio_pts = prev_audio_pts;
		tr->last_video_pts = prev_video_pts;
	}
